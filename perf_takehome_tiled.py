...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        for group_start in range(0, n_batches, GROUP_SIZE):
            ...
            # Load batch data ONCE per group (not per round!)
            ...
            for tile_start in range(0, rounds, ROUND_TILE):
                ...
                # Use diagonal scheduling across batches and rounds within the tile
                # Each batch progresses through rounds independently
                ...
                if need_scattered:
                    ...
                    if level == 0:
                        ...
                        if stage_done[g][STAGE_XOR] == -2:
                            if len(valu_ops) < 6:
                                valu_ops.append(("^", ctx["val"], ctx["val"], v_forest_nodes[0]))
                                stage_done[g][STAGE_XOR] = cycle
                            continue
                    elif level == 1:
                        # & -> vselect -> XOR
